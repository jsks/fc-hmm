...
#|          that when a civil conflict is conclusive terminated, it
...
Since $y_t \in \mathbb{N}_0$, I model the yearly battle related deaths given that $S_t$ is in state $k$ with a negative binomial distribution using the mean-dispersion parameterization,
...
Z_star <- fit$draws("Z_star", format = "matrix")
active_state <- Z_star[, sprintf("Z_star[%d,2]", 1:nrow(dataset))]
...
tiv <- fit$draws(c(sprintf("beta[1,%d,2]", 1:2), sprintf("beta[2,%d,2]", 1:2)),
                 format = "matrix")
