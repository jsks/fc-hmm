...
How do international arms transfers to incumbent governments affect the dynamics of civil conflict?
    - How do international arms transfers affect the probability of civil conflicts transitioning between frozen & active

one-sided international arms transfers
...
TODO: 3 state model, no restriction on transitions (ie 1 -> 3 and 1 -> 2)

TODO: hidden markov models
  - Allows us to estimate the probability that a conflict is in a particular state at a given time point (ie estimate latent states)
  - Allows us to model the effects of covariates on the probability of transitioning between latent states & model the effects of covariates on observable outcomes

Note: "a hidden markov model is a tool for representing probability distributions over sequences of observations"

Potential issues: computational costs, and multimodality in the posterior distribution, driven by the high variance in observed fatalities and the difficulty in identifying latent states.
    - One possible mitigation, informative priors. It pays to be bayesian. (Maybe compare bayesian approach to only EM algorithm).
...
such that $\mathbb{E}\left[y_{t} | S_t = k\right] = \mu_{ik}$ and $\text{Var}(y_{t} | S_t = k) = \mu_{ik} + \frac{\mu_{ik}^2}{\phi}$.[^negbin]

[^negbin]: This parameterization arises naturally from ... poisson-gamma mixture. As $\phi \to \infty$ then $y | S_t, \theta \overset{d}{\to} \text{Pois}$.
...
\begin{equation}
p(\theta | \boldsymbol{y}, X) \propto \prod_{i=1}^N p(y^{(i)}_{1:T_i} | X^{(i)}, \theta) p(\theta)
\end{equation}
...
Because of non-linear model, empirical estimands

average posterior predicted probability for transitioning from state $j$ to state $k$ for some constant TIV value across all transition cases.
...
Plots:
1. Predicted vs. actual deaths
2. Predicted probability of not being frozen
3. Parameter plot for beta tiv
4. APP - Marginal Effects
    - Prob. of a frozen conflict escalating
    - Prob. of an actie conflict de-escalating
5. (Maybe?) long-run effect of tiv (low vs medium vs high w/ covariates at means)
...
Zprob <- fit$draws("Gamma", format = "matrix") |> apply(2, median)
K <- fit$metadata()$stan_variable_sizes$beta[1]
for (i in 1:K) {
    state_regex <- \(i) sprintf("^Gamma\\[\\d+,%d\\]$", i)
    dataset[paste0("Zprob", i)] <- exp(Zprob[grepl(state_regex(i), names(Zprob))])
}

dataset$yhat <- fit$draws("yhat", format = "matrix") |> apply(2, median)

df <- select(dataset, unit_id, year, brd, yhat, matches("Zprob")) |>
    pivot_longer(cols = starts_with("Zprob"), names_to = "Z", values_to = "prob") |>
    mutate(Z = sub("^Zprob", "", Z) |> factor(levels = 3:1)) |>
    group_by(unit_id, year) |>
    mutate(prob = prob / sum(prob))

sub.df <- filter(df, unit_id == 63)

ggplot(sub.df, aes(year, brd, col = "Observed")) +
    geom_point() +
    geom_point(aes(year, yhat, col = "Predicted")) +
    geom_bar(aes(year, prob, fill = Z), stat = "identity", position = "stack", alpha = 0.6)

max_observed <- max(sub.df$brd)
scaling_factor <- 1 / max_observed

# Plot the stacked bar plot with the secondary y-axis
ggplot() +
  # Stacked bar plot (background)
  geom_tile(data = sub.df, aes(x = year, y = prob, fill = Z),
           stat = "identity", position = "stack", alpha = 0.2, width = 1) +
  # Observed count as points (overlay, scaled down to match y-axis)
    geom_point(data = sub.df, aes(x = year, y = brd * scaling_factor, col = "Observed")) +
    geom_point(data = sub.df, aes(year, yhat * scaling_factor, col = "Predicted")) +
  # Primary y-axis (for the probability simplex)
  scale_y_continuous(name = "Probability Simplex", sec.axis = sec_axis(~ . / scaling_factor, name = "Observed Count")
                     )  +
    theme_tufte()
